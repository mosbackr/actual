...
export default function ScoutPage() {
  ...
  async function handleSend() {
    ...
    setLoading(true);
    try {
      // No history — each request is standalone
      const result = await adminApi.scoutChat(session.backendToken, text, []);
    ...
    <div className="flex">
      ...
      <main className="ml-56 flex-1 flex flex-col h-screen">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {/* Quick search options — always visible */}
          <div className={`max-w-2xl mx-auto ${messages.length === 0 ? "flex flex-col items-center justify-center h-full" : "mb-6"}`}>
            {messages.length === 0 && (
              <>
            ...
            <div className="w-full space-y-3">
              {SOURCE_CATEGORIES.map((cat) => (
                <div key={cat.label}>
                  <h3 className="text-xs font-medium text-text-tertiary uppercase tracking-wide mb-1.5">{cat.label}</h3>
                  <div className="flex flex-wrap gap-1.5">
                    {cat.prompts.map((p) => (
                      <button
                        ...
                        className="text-xs px-2.5 py-1 rounded border border-border text-text-secondary hover:border-accent hover:text-accent transition"
                      >
                        {p.name}
                      </button>
                    ))}
